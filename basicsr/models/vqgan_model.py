...
class VQGANModel(SRModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('codebook_opt'):
            ...
            self.l_weight_codebook = 5.0        # best:5.0
    ...
    def optimize_parameters(self, current_iter, noise_std=0.01):
        logger = get_root_logger()
        ...
        if current_iter % self.net_d_iters == 0 and current_iter > self.net_g_start_iter:
            ...
            if current_iter > self.net_d_start_iter:
                ...
                loss_dict['l_g_gan'] = d_weight * l_g_gan       # 波动->上升

                loss_dict['d_weight'] = d_weight        # 下降
        ...
        if current_iter > self.net_d_start_iter:
            ...
            loss_dict['l_d_real'] = l_d_real        # 下降
            loss_dict['out_d_real'] = torch.mean(real_d_pred.detach())      # 上升
            l_d_real.backward()
            # fake
            fake_d_pred = self.net_d(self.output.detach())
            l_d_fake = self.cri_gan(fake_d_pred, False, is_disc=True)
            loss_dict['l_d_fake'] = l_d_fake        # 下降
            loss_dict['out_d_fake'] = torch.mean(fake_d_pred.detach())      # 下降

            loss_dict['l_d_gan'] = l_d_fake + l_d_real      # 下降
