...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        # Apply convolution to downsample
        x = self.conv(x)
        return x
...
class Encoder(nn.Module):
    def __init__(self, in_channels, nf, emb_dim, ch_mult, num_res_blocks, resolution, attn_resolutions):
        ...
        curr_res = self.resolution  # 256, 128, 64, 32, 16, 8, 4
        ...
        for i in range(self.num_resolutions):
            block_in_ch = nf * in_ch_mult[i]  # 64, 64, 128, 128, 256, 256
            block_out_ch = nf * ch_mult[i]  # 64, 128, 128, 256, 256, 512
...
class Generator(nn.Module):
    def __init__(self, nf, emb_dim, ch_mult, res_blocks, img_size, attn_resolutions):
        super().__init__()
        self.nf = nf
        self.ch_mult = ch_mult  # [1, 2, 2, 4, 4]
        self.num_resolutions = len(self.ch_mult)  # 5
        self.num_res_blocks = res_blocks  # 2
        self.resolution = img_size  # 256
        self.attn_resolutions = attn_resolutions  # 16
        self.in_channels = emb_dim  # 256
        self.out_channels = 3
        block_in_ch = self.nf * self.ch_mult[-1]  # 256, 256, 256, 128, 128, 64
        curr_res = self.resolution // 2 ** (self.num_resolutions - 1)  # 16, 32, 64, 128, 256
        ...
        for i in reversed(range(self.num_resolutions)):
            block_out_ch = self.nf * self.ch_mult[i]  # 256, 256, 128, 128, 64
            for _ in range(self.num_res_blocks):
                blocks.append(ResBlock(block_in_ch, block_out_ch))
                block_in_ch = block_out_ch
...
class VQAutoEncoder(nn.Module):
    ...
    def forward(self, x):
        _, _, h, w = x.shape
        x, fea_size = self.hq_encoder(x)
        quant, codebook_loss, quant_stats = self.quantize(x)
        x = self.generator(quant, fea_size)
